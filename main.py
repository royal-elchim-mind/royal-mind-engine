...
def get_history_from_db(phone, limit=10):
    ...
    history = " | ".join([f"[تاريخ {r['date']} - {r['type']}]: {r['text']}" for r in reversed(records) if str(r.get('phone')).strip() == clean_phone])
    return history
...
async def get_vault(phone: str):
    ...
    clean_phone = str(phone).strip()
    if not clean_phone: return {"status": "success", "data": []}

    # الفلترة المطلقة باستخدام $eq لضمان عدم تداخل السجلات
...
async def craft_perfume(payload: PerfumeCraftPayload):
    try:
        ...
        perfume_prompt = (
            f"{BASE_PHILOSOPHY}\nالتاريخ التراكمي: [{history_context}]\nرسالة العميل: '{payload.client_message}'\n\n"
